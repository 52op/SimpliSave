...
export async function handleCreateBookmark(request: Request, env: Env): Promise<Response> {
...
export async function handleUpdateBookmark(request: Request, env: Env, id: string): Promise<Response> {
  const userId = await getUserId(request);
  if (!userId) return errorResponse('Unauthorized', 401);
...
export async function handleDeleteBookmark(request: Request, env: Env, id: string): Promise<Response> {
   const userId = await getUserId(request);
   if (!userId) return errorResponse('Unauthorized', 401);
 ...
 export async function handleSearchBookmarks(request: Request, env: Env): Promise<Response> {
   const url = new URL(request.url);
   const q = url.searchParams.get('q') || '';
   const userId = await getUserId(request);
