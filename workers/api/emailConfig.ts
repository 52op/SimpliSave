...
export async function handleGetEmailConfig(request: Request, env: any): Promise<Response> {
  ...
  const row = await env.DB
    .prepare('SELECT id, provider, from_address, domain, region, enabled, updated_at FROM email_config ORDER BY updated_at DESC LIMIT 1')
    .first()
...
export async function handleUpdateEmailConfig(request: Request, env: any): Promise<Response> {
  ...
  let body: { provider?: Provider; api_key?: string; from_address?: string; domain?: string; region?: string; enabled?: boolean }
  try {
  ...
  const { provider, api_key, from_address, domain, region, enabled = true } = body
  if (!provider || !from_address) return errorResponse('provider 和 from_address 为必填项', 400)
  if (!(['resend', 'sendgrid', 'mailgun'] as Provider[]).includes(provider)) return errorResponse('无效的 provider', 400)
  ...
  if (existing) {
    await env.DB.prepare(
      `UPDATE email_config SET provider=?, api_key=?, from_address=?, domain=?, region=?, enabled=?, updated_at=? WHERE id=?`
    ).bind(provider, finalApiKey, from_address, domain ?? null, region ?? 'us', enabled ? 1 : 0, now, existing.id).run()
  } else {
    await env.DB.prepare(
      `INSERT INTO email_config (provider, api_key, from_address, domain, region, enabled, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)`
    ).bind(provider, finalApiKey, from_address, domain ?? null, region ?? 'us', 1, now).run()
  }
