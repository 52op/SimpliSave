...
---
### 如何让非托管在cloudflare的域名cname到workers项目域名不报1001

#### 方案一：使用 Routes（路由）触发（最常用、最直接）

这是给非托管域名绑定 Workers 的经典操作。Workers 除了“Custom Domains”这种独占域名的模式外，还支持 **Routes（路由）** 模式。

1. **去你的外部 DNS 解析商（如腾讯云/阿里云）：**
   
   - 给 `simplisave-api.it0731.cn` 添加一条 **CNAME** 记录。
   
   - 目标（Target）**不要**填 `*.workers.dev`（会报 1001 错误）。
   
   - 目标（Target）直接填你**已经成功托管在 CF 上且激活的任意域名**。例如，填你 Pages 刚刚绑好的生产域名：`simplisave.letvar.cc.cd`（或者直接填 `letvar.cc.cd`）。

> **原理：** 只要把流量通过 CNAME 先骗到 Cloudflare 的网络节点（任一属于你的、已开橙云的域名），Cloudflare 就能接管这个请求。

2. **回到 Cloudflare 你的 Workers 项目后台：**
   
   - 切换到 **Settings** -> **Domains & Routes**。
   
   - 在下面的 **Routes** 区域，点击 **Add Route**。
   
   - 在 Route 框中直接手动输入外部域名：`simplisave-api.it0731.cn/*`
   
   - Zone（区域）下拉菜单中，选择你托管在 CF 的那个域名：`letvar.cc.cd`。
   
   - 点击保存。

这样，当请求到达 `simplisave-api.it0731.cn` 时，因为 CNAME 到了 CF 节点，CF 的边缘网关会在 `letvar.cc.cd` 区域的路由表里匹配到这条 Route 规则，从而完美触发你的 Workers。

#### 方案二：利用 Cloudflare for SaaS（官方标准解法）

如果你希望像 Pages 一样，享受专属的 SSL 证书自动颁发、独立域名回源，且不想搞复杂的 CNAME 借道，那么就需要用到 **Cloudflare for SaaS（Custom Hostnames）**。这个功能现在对免费账户也开放了每月 100 个免费额度。

1. **设置回源后门（Fallback Origin）：**
   
   - 点击进入你托管在 CF 的域名 `letvar.cc.cd` 的控制台。
   
   - 依次点击左侧菜单的 **SSL/TLS** -> **Custom Hostnames**。
   
   - 在 **Fallback Origin** 位置，输入你的 Workers 域名（例如：`your-worker-name.your-subdomain.workers.dev`），然后点击 **Submmit**。
   
   - *注：这里是唯一允许把外部请求无缝路由给 workers.dev 域名且不触发 1001 错误的地方。*

2. **添加你的外部域名：**
   
   - 还在当前页面，点击 **Add Custom Hostname**。
   
   - **Custom Hostname** 填入：`simplisave-api.it0731.cn`。
   
   - 证书和所有权验证选择 **TXT**（或者选择 HTTP 验证）。

3. **去外部 DNS 解析商处配置：**
   
   - 按照 Cloudflare 页面上生成的提示，去你 `it0731.cn` 的 DNS 后台添加相应的 `TXT` 验证记录（用于颁发 SSL 证书）。
   
   - 将 `simplisave-api.it0731.cn` 的 **CNAME** 指向你在 CF 的回源跳板（即 `letvar.cc.cd` 分配给你的高防节点）。

### 总结建议

- 如果你只是为了写个接口，想快速搞定上线，用 **【方案一（Routes）】** 最快：外部 DNS 随便 CNAME 到你一个能通的橙云域名，然后在 Workers 里加一条 `外部域名/*` 的 Route 即可。

- 如果对 SSL 证书的合规性、以及未来的扩展性有要求，用 **【方案二（SaaS）】** 则是最规整、彻底的解法。
---
