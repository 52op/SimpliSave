...
export default function SearchModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const doSearch = useCallback(async (q: string) => {
    ...
    try {
      ...
      // 公开子链接去重（同一 URL 可能存在于多个卡片组），保留各条目但按 URL 去重只展示一次
      ...
      for (const b of (bms || [])) {
        ...
        all.push({
          ...
          icon_url: b.icon_url,
          group_slug: (b as any).group_slug || null,
          group_title: b.group_title || null,
        })
      }
      for (const g of (grps || [])) {
  ...
  function handleSelect(result: SearchResult) {
    ...
    switch (result.type) {
      case "bookmark":
        // 跳转到所属卡片组页面，无卡片组信息时才直接打开链接
        if (result.group_slug) navigate(`/g/${result.group_slug}`)
          ...
          {!loading && results.length > 0 && results.map((r, i) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {r.type === "bookmark" && r.group_title && (
                  <p className="text-xs text-blue-500 dark:text-blue-400 truncate mt-0.5 flex items-center gap-1">
                    <FolderOpen className="w-3 h-3 shrink-0" />{r.group_title}
                  </p>
                )}
                {r.description && r.type !== "bookmark" && <p className="text-xs text-[var(--color-text-muted)] truncate mt-0.5">{r.description}</p>}
              </div>
